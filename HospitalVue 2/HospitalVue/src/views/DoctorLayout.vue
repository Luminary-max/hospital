      ...
      <el-col :span="12">
        <el-card shadow="hover" class="stat-card">
          <div class="stat-inner">
            ...
            <div class="stat-info"><div class="stat-num">{{ orderPeople }}</div><div class="stat-label">今日预约挂号人数</div></div>
          </div>
        </el-card>
      </el-col>
      <el-col :span="12">
        <el-card shadow="hover" class="stat-card">
          <div class="stat-inner">
            ...
            <div class="stat-info"><div class="stat-num">{{ orderPeople }}</div><div class="stat-label">我的门诊量</div></div>
          </div>
        </el-card>
      </el-col>
    </el-row>
...
export default {
  name: "DoctorLayout",
  data() { return { userId: "", orderPeople: 0 }; },
  methods: {
    tokenDecode(t) { if(t) return jwtDecode(t); },
    requestPeople() { request.get("order/orderPeopleByDid",{params:{dId:this.userId}}).then(r=>{if(r.data.status===200)this.orderPeople=r.data.data||0;}); }
  },
  created() { const d=this.tokenDecode(getToken()); if(d) this.userId=d.dId; this.requestPeople(); }
};
</script>
<style scoped>
